...
class ArchonContent
{
    ...
    public function exportData($data)
    {
    	...
    	$writer_content = Writer::createFromPath('/home/vagrant/code/archon2atom/storage/app/data_export/collectioncontent.csv', 'w+');
    	$writer_content->insertOne($header['content']);
		$writer_content->insertAll($data['content']); //using an array

		if(!is_null($data['subjects']))
		{
			$writer_subjects = Writer::createFromPath('/home/vagrant/code/archon2atom/storage/app/data_export/collectioncontent-subjects.csv', 'w+');
		...
		{
			$writer_notes = Writer::createFromPath('/home/vagrant/code/archon2atom/storage/app/data_export/collectioncontent-notes.csv', 'w+');
    		$writer_notes->insertOne($header['notes']);
			$writer_notes->insertAll($data['notes']); //using an array
		}

		if(!is_null($data['creators']))
		{
			$writer_creators = Writer::createFromPath('/home/vagrant/code/archon2atom/storage/app/data_export/collectioncontent-creators.csv', 'w+');
